...
namespace TengoDotNetCore.API.WXPay {
    public class WxPayApi {
        ...
        * @return 成功时返回，其他抛异常
        */
        public static WxPayData UnifiedOrder(WxPayData inputObj, int timeOut = 6, int device = 2, string appid = "") {
            ...
            //其实用哪个都是可以的，只不过如果用了同一个回调地址的话，就无法追踪到底是用微信下具体的哪个支付了
            //当然，要做也是可以的，可能会稍微麻烦点
            if (!inputObj.IsSet("notify_url")) {
                // 设备号2表示手机端（默认）,异步通知地址不同 
                inputObj.SetValue("notify_url", WxPayConfig.NOTIFY_URL);//异步通知url

            }
            //设置APPID，如果传有的话，就用传过来的，不然就用微信支付共通的
            inputObj.SetValue("appid", string.IsNullOrWhiteSpace(appid) ? WxPayConfig.APPID : appid);//公众账号ID
            inputObj.SetValue("mch_id", WxPayConfig.MCHID);//商户号

            //如果不是H5支付的话，随便取ip
            if (inputObj.GetValue("trade_type").ToString() != "MWEB") {
                inputObj.SetValue("spbill_create_ip", WxPayConfig.IP);//终端ip	  	    
            }
            ...
            //设置微信原始请求
            result.InitalRequest = url + "?" + xml;
            //设置微信原始响应
            result.InitalResponse = response;
