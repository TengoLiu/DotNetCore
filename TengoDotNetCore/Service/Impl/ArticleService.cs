...
namespace TengoDotNetCore.Service.Impl {
    public class ArticleService : AbsService, IArticleService {
        ...
        public async Task<JsonResultObj> CategoryAdd(ArticleCategory model) {
            try {
        ...
        public async Task<ArticleCategory> CategoryDetail(int? ID) {
            if (ID == null || ID <= 0) {
                return null;
            }
        ...
        public async Task<JsonResultObj> CategoryEdit(ArticleCategory model) {
            try {
                db.Entry(model).Property(p => p.Title).IsModified = true;
                db.Entry(model).Property(p => p.CoverImg).IsModified = true;
                db.Entry(model).Property(p => p.Sort).IsModified = true;
